...
export class IntelliSenseManager {
  ...
  private analyzeAndRegisterComponents(code: string): void {
    ...
    while ((match = namedImportPattern.exec(code)) !== null) {
      ...
      const componentNames = importedNames.filter(name => /^[A-Z]/.test(name));
      ...
      if (componentNames.length > 0) {
        ...
        componentNames.forEach(name => {
          ...
          if (moduleName.includes("vapor-ui") || moduleName.includes("@vapor-ui")) {
            // vapor-ui 컴포넌트의 경우
  ...
  private getVaporUIComponentProps(componentName: string): string {
    const vaporUIPropsMap: Record<string, string> = {
      Button: "{ variant?: 'solid' | 'outline' | 'ghost'; colorScheme?: string; size?: 'xs' | 'sm' | 'md' | 'lg'; isLoading?: boolean; onClick?: () => void; children: React.ReactNode }",
      Input: "{ variant?: 'outline' | 'filled' | 'flushed'; size?: 'xs' | 'sm' | 'md' | 'lg'; isInvalid?: boolean; placeholder?: string; value?: string; onChange?: (e: React.ChangeEvent<HTMLInputElement>) => void }",
      Box: "{ p?: number | string; m?: number | string; bg?: string; w?: string | number; h?: string | number; children?: React.ReactNode }",
      Stack: "{ spacing?: number | string; direction?: 'row' | 'column'; align?: 'start' | 'center' | 'end'; children: React.ReactNode }",
      Text: "{ fontSize?: string; color?: string; fontWeight?: string; children: React.ReactNode }",
      Heading: "{ size?: 'xs' | 'sm' | 'md' | 'lg' | 'xl' | '2xl'; color?: string; children: React.ReactNode }",
      ...
      Modal: "{ isOpen: boolean; onClose: () => void; size?: 'xs' | 'sm' | 'md' | 'lg' | 'xl'; children: React.ReactNode }",
      Alert: "{ status?: 'success' | 'error' | 'warning' | 'info'; variant?: 'solid' | 'subtle' | 'outline'; children: React.ReactNode }",
    };

    return vaporUIPropsMap[componentName] || "{ children?: React.ReactNode; [key: string]: any }";
  }
...
declare global {
  ...
  private async registerCommonComponents(): Promise<void> {
    ...
    const basicComponents = {
      Button: "React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: 'primary' | 'secondary' | 'outline'; size?: 'sm' | 'md' | 'lg' }",
      Input: "React.InputHTMLAttributes<HTMLInputElement> & { variant?: 'outline' | 'filled'; error?: boolean }",
      Card: "React.HTMLAttributes<HTMLDivElement> & { variant?: 'elevated' | 'outlined' }",
      Modal: "{ isOpen: boolean; onClose: () => void; title?: string; children: React.ReactNode }",
      Tooltip: "{ content: string; placement?: 'top' | 'bottom' | 'left' | 'right'; children: React.ReactElement }",
      Spinner: "{ size?: 'sm' | 'md' | 'lg'; color?: string }",
      Alert: "{ type?: 'info' | 'success' | 'warning' | 'error'; title?: string; children: React.ReactNode }",
      Badge: "{ variant?: 'primary' | 'secondary' | 'success' | 'warning' | 'error'; children: React.ReactNode }",
      Avatar: "{ src?: string; alt?: string; size?: 'sm' | 'md' | 'lg'; fallback?: string }",
      Dropdown: "{ trigger: React.ReactElement; children: React.ReactNode; placement?: 'bottom-start' | 'bottom-end' }",
    };
    ...
    const vaporUIComponents = {
      VButton: "{ variant?: 'solid' | 'outline' | 'ghost'; colorScheme?: string; size?: 'xs' | 'sm' | 'md' | 'lg'; isLoading?: boolean; children: React.ReactNode }",
      VInput: "{ variant?: 'outline' | 'filled' | 'flushed'; size?: 'xs' | 'sm' | 'md' | 'lg'; isInvalid?: boolean; placeholder?: string }",
      VBox: "{ p?: number | string; m?: number | string; bg?: string; children?: React.ReactNode }",
      VStack: "{ spacing?: number | string; align?: 'start' | 'center' | 'end'; children: React.ReactNode }",
      VText: "{ fontSize?: string; color?: string; fontWeight?: string; children: React.ReactNode }",
      VHeading: "{ size?: 'xs' | 'sm' | 'md' | 'lg' | 'xl'; color?: string; children: React.ReactNode }",
      VFlex: "{ direction?: 'row' | 'column'; align?: 'start' | 'center' | 'end'; justify?: 'start' | 'center' | 'end' | 'between'; children: React.ReactNode }",
    };
    ...
    patterns.forEach((pattern, index) => {
      ...
      while ((match = pattern.exec(code)) !== null) {
        ...
        if (
          ...
          console.log(`Adding import: "${cleanModuleName}" (from "${moduleName}")`);
